...
const Finance = () => {
  ...
  const [dailyWisdom, setDailyWisdom] = useState<string>("Wealth is not about having more money—it's about having more options. Build your financial fortress one brick at a time.");
  ...
  const getDailyFinancialWisdom = () => {
    const wisdomList = ["Wealth is not about having more money—it's about having more options. Build your financial fortress one brick at a time.", "The market is driven by emotion, but wealth is built through logic. Stay rational when others panic.", "Your net worth is not your self-worth, but financial freedom gives you the power to help others.", "Invest in assets that generate income while you sleep. Passive income is the path to true freedom.", "The biggest risk is not taking any risk at all. Calculated risks lead to extraordinary returns.", "Time in the market beats timing the market. Consistency trumps perfection every time.", "Your money should work harder than you do. Make every dollar a soldier in your wealth army.", "Debt is a tool when used wisely, a weapon when used foolishly. Master the difference.", "Financial intelligence is more valuable than a high income. Learn the language of money.", "Build multiple income streams like you build multiple escape routes. Never depend on just one."];
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="flex justify-center mb-12">
            <div className="bg-gotham-gray rounded-full p-1">
              ...
              <button onClick={() => setMode('wayne')} className={`px-6 py-3 rounded-full font-batman font-bold text-sm uppercase tracking-wide transition-all duration-300 ${mode === 'wayne' ? 'bg-bat-yellow text-gotham-black' : 'text-gray-400 hover:text-white'}`}>ADVANCED</button>
            </div>
          </div>
          ...
          {loading ? <div className="flex justify-center items-center py-20">
              ...
              {currentTips.map((tip, index) => <div key={index} className="hover:transform hover:scale-105 transition-all duration-300">
                  <WisdomCard title={tip.title} quote={tip.quote} category={tip.category} icon={tip.icon} />
                </div>)}
            </div>}
          ...
          <div className="gotham-card p-8 rounded-lg text-center">
            ...
            <div className="flex justify-center space-x-4">
              <a href="https://drive.google.com/file/d/1sDWlNWjrl-zLlc0EVIxe_Ix9xFwGEpJU/view?usp=drive_link" target="_blank" rel="noopener noreferrer" className="px-6 py-3 rounded-full border border-bat-yellow text-bat-yellow hover:bg-bat-yellow hover:text-gotham-black transition-all duration-300 font-batman font-bold text-sm uppercase tracking-wide">
                Investment Guide
              </a>
            </div>
          </div>
